...
import forms
from DEC21.Week_10.Day_3.API import data

directory = "/Week 11/Day 5/static/img"
...
data_dict = json.loads("/product_db.json")
print(data_dict)

product = data_dict.json()['Name']
...
def read():
    with open('/product_db.json') as json_file:
    ...
    with open('/product_db.json') as json_file:
        ...
        for item in data:
            if item["ProductId"] == idn:
                ...
                price = item["Price"]
                return render_template('productdetail.html', name=name, idn=idn, right=right, catego=catego,
                                       brand=brand, status=status, quantity=quantity, price=price)
